...
class WordStudy extends Frame {
	...
	void display(int pos) { // pos위치에 있는 라인의 내용을 보여준다.
		/*
		 * (1) wordList에서 pos번째의 위치에 있는 데이터를 읽어온다. (2) StringTokenizer를 이용해서
		 * ‘|’를 구분자로 자른다. (3) 잘라진 Token에 개행문자(CR_LF)를 붙여서 StringBuffer에
		 * 붙인다.(append) (4) StringBuffer의 내용을 뽑아서 TextArea에 보여준다.
		 */
	}

	void loadFile(String fileName) {
		/*
		 * (1) BuffredReader와 FileReader를 이용해서 파일의 내용을 라인 단위로 읽는다. (2) 읽어온 라인을
		 * wordList에 저장한다. (3) 만일 예외가 발생하면 프로그램을 종료한다.
		 */
	}
