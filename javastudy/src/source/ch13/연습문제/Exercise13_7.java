...
class Exercise13_7 extends Frame implements MouseListener {
	...
	public void mousePressed(MouseEvent e) { // MouseListener
		...
		/*
		 * (1) 알맞은 코드를 넣어 완성하시오. 1. x와 y의 값을 클릭한 곳에서 가장 가까운 교차점으로 변경한다.(반올림) 2.
		 * x와 y의 값에서 돌의 크기(STONE_SIZE)의 절반을 빼야 클릭한 곳에 돌이 그려진다. 3. x와 y의 값이 오목판의
		 * 밖을 벗어난 곳이면 돌을 그리지 않는다. 4. 눌러진 버튼이 마우스 왼쪽 버튼이면, (x,y)의 위치에 검은 돌을 그리고
		 * 눌러진 버튼이 마우스 오른쪽 버튼이면, (x,y)의 위치에 흰 돌을 그린다. (흰 돌을 그릴 때 검은 색 테두리도 같이
		 * 그린다.) 5. repaint()를 호출한다.
		 */
	}
