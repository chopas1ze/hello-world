...
class EventTest {
	...
	static void addAllListener(Component c) {
		...
		class EventHandler implements ActionListener, AdjustmentListener, ComponentListener, ContainerListener,
				FocusListener, ItemListener, KeyListener, MouseListener, MouseMotionListener, MouseWheelListener,
			...
			public void adjustmentValueChanged(AdjustmentEvent e) {
				print("adjustmentValueChanged", e);
			}

			// ComponentListener의 메서드
			public void componentResized(ComponentEvent e) {
				print("componentResized", e);
			}

			public void componentMoved(ComponentEvent e) {
				print("componentMoved", e);
			}

			public void componentShown(ComponentEvent e) {
				print("componentShown", e);
			}

			public void componentHidden(ComponentEvent e) {
				print("componentHidden", e);
			}

			// ContainerListener의 메서드
			public void componentAdded(ContainerEvent e) {
				print("componentAdded", e);
			}

			public void componentRemoved(ContainerEvent e) {
				print("componentRemoved", e);
			}
			...
			public void mouseWheelMoved(MouseWheelEvent e) {
				print("mouseWheelMoved", e);
			}

			// TextListener의 메서드
			public void textValueChanged(TextEvent e) {
				print("textValueChanged", e);
			}

			// WindowFocusListener의 메서드(JDK1.4부터추가)
			public void windowGainedFocus(WindowEvent e) {
				print("windowGainedFocus", e);
			}

			public void windowLostFocus(WindowEvent e) {
				print("windowLostFocus", e);
			}

			// WindowListener의 메서드
			public void windowOpened(WindowEvent e) {
				print("windowOpened", e);
			}
			...
			public void windowClosed(WindowEvent e) {
				print("windowClosed", e);
			}

			public void windowIconified(WindowEvent e) {
				print("windowIconified", e);
			}

			public void windowDeiconified(WindowEvent e) {
				print("windowDeiconified", e);
			}

			public void windowActivated(WindowEvent e) {
				print("windowActivated", e);
			}

			public void windowDeactivated(WindowEvent e) {
				print("windowDeactivated", e);
			}

			// WindowStateListener의 메서드(JDK1.4부터추가)
			public void windowStateChanged(WindowEvent e) {
				print("windowStateChanged", e);
			}
		...
		if (c instanceof Container) {
			...
			((Button) c).addActionListener(handler);
			// } else if (c instanceof Canvas) { // 따로 추가해줄 Listener가 없다.
		} else if (c instanceof Checkbox) {
			((Checkbox) c).addItemListener(handler);
		} else if (c instanceof Choice) {
			((Choice) c).addItemListener(handler);
			...
			((Scrollbar) c).addAdjustmentListener(handler);
		} else if (c instanceof TextArea) {
			((TextArea) c).addTextListener(handler);
