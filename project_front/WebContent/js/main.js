$(function() {
			...
			.Swipe(
					{
						...
						callback : function(index, element) { // 이벤트가 완료되면
							// 실행합니다.

							/*
							 * 클래스 "active"를 포함하는 불릿 버튼을 비활성화 버튼으로 만들고, "active"
							 * 클래스를 삭제합니다.
							 */
							...
							/*
							 * 인덱스(index)에 해당하는 불릿 버튼을 활성화된 버튼으로 만들고, "active"
							 * 클래스를 생성합니다.
							 */
	...
	$("#roll_banner_wrap dt a").on(
			"click",
			function() {// 1~4버튼을 클릭할 때...
				$("#roll_banner_wrap dd:visible").hide(); // 노출되어있는 배너를 숨깁니다.

				/*
				 * onBtn에 하위 요소 <img>에 "src"속성을 비활성화된 이미지 경로로 바꿉니다.
				 */
				$("img", onBtn).attr(
						"src",
	...
	function autoPlay() {// 1~4버튼이 순서대로 강제 클릭 이벤트가 발생되는 함수
		...
		/*
		 * 4초 간격으로 재귀함수 호출이 발생합니다. 그러므로 autoPlay()함수에 일련의 실행문이 반복 실행됩니다.
		 */
	...
	$("#tabmenu dt a").on(
			...
			function() {
				...
				/*
				 * onTab에 할당된 요소의 하위 <img>에 "src" 속성을 비활성화 버튼이미지로 바꿉니다.
				 */
				$("img", onTab).attr(
						"src",
