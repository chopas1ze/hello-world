...
/*
 * awt에서 제공된 컴포넌트을 swing에서 상속받아 사용할때 : button -> JButton
...
class SwingComponent extends JFrame implements ActionListener {
	...
	public SwingComponent() {
		...
		inputTxt = new JTextField(30); //입력받는 글자수
		multiArea = new JTextArea();  //아무런 text도 없는 빈 TextArea를 생성한다.
		JPanel p = new JPanel(); //여러개의 컴퍼넌트를 그룹화시킴.  //FlowLayout(왼쪽->오른쪽)
		p.add(inputTxt);
		p.add(runBtn);
