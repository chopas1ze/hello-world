...
// ctrl + shift + O(영문)  : 여러개 import
// 클래스명이나 인터페이스명 끝글자에 커서 대고 컨트롤  + 스페이스 : 한개만 import
/*
 * 인터페이스(interface)
...
interface CarRun {
	int car_count = 0; // public static final 컴파일과정중에 자동으로 처리된다.

	void work(); // public abstract 컴파일과정중에 자동으로 처리된다.
}

class TrunkRun implements CarRun, Serializable { // 다중 상속 가능
